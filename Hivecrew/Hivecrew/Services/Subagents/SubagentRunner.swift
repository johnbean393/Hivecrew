...
final class SubagentRunner {
    ...
    private func callLLMWithRetry(
        ...
        for attempt in 1...maxLLMRetries {
            do {
                ...
                if let compactionReason = ContextCompactionPolicy.compactionReason(for: error),
                   contextCompactionRetries < maxContextCompactionRetries {
                    ...
                    if isPayloadTooLargeError(error) {
                        if let nextLevel = workingScaleLevel.next {
                            onLine?(ProgressLine(
                                type: .info,
                                summary: "Context compaction (\(compactionReason.rawValue)): downscaling images to \(nextLevel)",
                                details: nil
                            ))
                        ...
                        onLine?(ProgressLine(
                            type: .info,
                            summary: "Context compaction (\(compactionReason.rawValue)): removing older images",
                            details: nil
                        ))
                    ...
                    onLine?(ProgressLine(
                        type: .info,
                        summary: "Context compaction (\(compactionReason.rawValue)): compacting messages",
                        details: nil
                    ))
                ...
                if isEmptyResponseError(error), attempt < maxLLMRetries {
                    ...
                    onLine?(ProgressLine(
                        type: .info,
                        summary: "LLM returned empty response (\(attempt)/\(maxLLMRetries)); retrying in \(Int(delay))s",
                        details: nil
                    ))
                ...
                if isRetryableError(error), attempt < maxLLMRetries {
                    ...
                    onLine?(ProgressLine(
                        type: .info,
                        summary: "Retryable LLM failure (\(attempt)/\(maxLLMRetries)); retrying in \(Int(delay))s",
        ...
        throw lastError ?? LLMError.unknown(message: "LLM call failed after \(maxLLMRetries) attempts")
    }
    ...
    private func proactivelyCompactMessagesIfNeeded(
        ...
        for pass in 1...maxPasses {
            ...
            onLine?(ProgressLine(
                type: .info,
                summary: "Context compaction (threshold85): \(estimated)/\(maxInputTokens) tokens (\(fillPercent)% full)",
                details: nil
            ))
            ...
            if !changed {
                onLine?(ProgressLine(
                    type: .info,
                    summary: "Context still high, but no further compaction was possible",
                    details: nil
                ))
                return
            }
            ...
            onLine?(ProgressLine(
                type: .info,
                summary: "Context compaction pass \(pass) -> \(afterEstimate)/\(maxInputTokens) tokens (\(afterFill)% full)",
                details: nil
            ))
        }
    }
    ...
    private func generateCompactionSummaryUsingLLM(
        ...
        do {
            ...
            onLine?(ProgressLine(
                type: .info,
                summary: "LLM summary compaction failed; using heuristic compaction",
                details: error.localizedDescription
            ))
            return nil
        }
    ...
    private func learnContextBudget(from error: Error) async -> ContextBudget? {
        ...
        if let learnedLimit = learned?.maxInputTokens {
            onLine?(ProgressLine(
                type: .info,
                summary: "Learned context budget from provider error: \(learnedLimit) tokens",
                details: nil
            ))
        }
        return learned
